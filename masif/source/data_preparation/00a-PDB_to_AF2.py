...
This will:
  ...
  - Save RMSD results to ./data/rmsd/7MON_rmsd.json
    ...
    if args.verbose:
        print(f"\nDownloading PDB file : {pdb_file_path}")
    # Load the structure
    if args.verbose:
    ...
    # Use the user-specified (or default) pdb-out-dir, which was already created above
    pdb_out_dir = args.pdb_out_dir
    ...
    # Build and save a combined complex PDB with all chains, as expected by downstream MaSIF scripts
    complex_struct = Structure.Structure('aligned_af2_complex')
    complex_model = Model.Model(0)
    complex_struct.add(complex_model)

    # Add chains from interactor 1
    for ch in interactor_1_af2_aligned_struct.get_chains():
        chain_id = ch.id
        if chain_id in complex_model:
            complex_model.detach_child(chain_id)
        complex_model.add(ch)

    # Add chains from interactor 2
    for ch in interactor_2_af2_aligned_struct.get_chains():
        chain_id = ch.id
        if chain_id in complex_model:
            complex_model.detach_child(chain_id)
        complex_model.add(ch)

    complex_pdb_path = os.path.join(pdb_out_dir, f'{pdb_id}.pdb')
    io_complex = PDBIO()
    io_complex.set_structure(complex_struct)
    io_complex.save(complex_pdb_path)
    print(f"  Saved complex PDB {complex_pdb_path}")
