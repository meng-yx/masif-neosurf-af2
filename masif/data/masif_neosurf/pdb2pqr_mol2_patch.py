...
class Mol2Molecule:
    ...
    @staticmethod
    def remove_fused_rings(rings):
        """
        Find smallest ring for each atom and return the union of all these smallest rings as the set of unfused rings.
        """
        rings = set(tuple(sorted(r)) for r in rings)  # de-duplicate rings if necessary
        atoms = set().union(*rings)
        unfused_rings = set()
        for a in atoms:
            smallest_ring = None
            for r in rings:
                if a in r and ((smallest_ring is None) or (len(r) < len(smallest_ring))):
                    smallest_ring = r
            assert smallest_ring is not None
            unfused_rings.add(smallest_ring)
        
        return unfused_rings
    ...
    def set_rings(self):
        ...
        #         self.rings.add(ring)
        self.rings = self.remove_fused_rings(rings)
